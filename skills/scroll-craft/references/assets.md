...
Generation through **Magnific** (preferred) or kie.ai (fallback), then encoding
for scrubbing. Two generator scripts share the same CLI shape —
`scripts/magnific.mjs` and `scripts/kie.mjs` — plus `scripts/encode.sh` for
both. Use whichever key is set; if `MAGNIFIC_API_KEY` is present, use
`magnific.mjs`. Fall back to `kie.mjs` only if just `KIE_AI_API_KEY` is set.

```bash
node <skill>/scripts/magnific.mjs probe                     # confirms the key
node <skill>/scripts/magnific.mjs still "<prompt>" out/01.png --ar 16:9 [--ref brand.png]
node <skill>/scripts/magnific.mjs shot  "<move>"   out/01.png out/01.mp4 --dur 5
bash  <skill>/scripts/encode.sh out/01.mp4 assets/01.mp4
bash  <skill>/scripts/encode.sh out/01.mp4 assets/01-m.mp4 mobile
```

Models on Magnific: `nano-banana-pro-flash` for stills (image-to-image when you
pass `--ref`), `kling-v2-1-master` for camera moves. Magnific's Kling 2.1 Master
endpoint takes one input image, not a head/tail pair, so `magnific.mjs shot`
has no `--tail` — see the note at the top of the script if a build genuinely
needs pinned first/last-frame chaining (Kling O1, a different endpoint).

The kie.ai path (`kie.mjs`) still supports `--tail` for that case:
...
kie.ai models: `seedream/5-pro-text-to-image` (and `-image-to-image` when you pass
